...
from pyspark_traffic_pipeline import (
    TPPP_ANOMALY_FILE,
    TPPP_CURATED_DIR,
    TPPP_SUMMARY_FILE,
...
@st.cache_data(show_spinner=False)
def load_tppp_outputs(summary_path: str, anomaly_path: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Load real PySpark TPPP output CSV files for the frontend."""
    return load_tppp_summary(Path(summary_path)), load_tppp_anomalies(Path(anomaly_path))
...
def main() -> None:
    ...
    st.markdown(
        """
        <div class="hero">
            <div>⚡ SMART CITY BIG DATA + REAL PYSPARK TPPP</div>
            <h1>Quantum Traffic Flow Anomaly Analytics System</h1>
            <p>Analyze 100,000+ synthetic vehicle events, run a real PySpark Traffic Preprocessing
            & Profiling Pipeline, and explore ICU batch simulation outputs from one futuristic frontend.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )

    traffic_tab, tppp_tab, icu_tab, data_hub_tab = st.tabs([
        "🚦 Traffic Command Center",
        "⚙️ TPPP Real PySpark",
        "🏥 ICU Batch Simulation",
        "🧬 Synthetic Data Hub",
    ])
    ...
    with tppp_tab:
        st.markdown('<h3 class="section-title">TPPP: Traffic PySpark Preprocessing Pipeline</h3>', unsafe_allow_html=True)
        st.caption(
            "This is a real PySpark batch job. It starts a local SparkSession, reads the generated traffic CSV, "
            "builds risk features with Spark SQL, computes rolling window metrics, writes curated Parquet, "
            "and exports frontend-ready CSV summaries."
        )

        spark_available = is_pyspark_available()
        status_cols = st.columns(4)
        with status_cols[0]:
            metric_card("PySpark status", "Ready" if spark_available else "Missing", "Install pyspark to run")
        with status_cols[1]:
            metric_card("Input CSV", DATA_FILE.name, "Traffic telemetry source")
        with status_cols[2]:
            metric_card("Curated output", "Parquet", str(TPPP_CURATED_DIR))
        with status_cols[3]:
            metric_card("Useful job", "Risk profiling", "Zone/weather/anomaly batch")

        if not spark_available:
            st.warning("PySpark is not installed in this environment. Run `pip install -r requirements.txt` and then use this tab.")

        run_col, log_col = st.columns((1, 1.25))
        with run_col:
            anomaly_limit = st.slider("Top anomalies exported by Spark", min_value=25, max_value=500, value=100, step=25)
            if st.button("Run TPPP Real PySpark Job", use_container_width=True, disabled=not spark_available):
                try:
                    with st.spinner("Starting local SparkSession and processing traffic data..."):
                        result = run_traffic_pyspark_pipeline(anomaly_limit=anomaly_limit)
                        load_tppp_outputs.clear()
                    st.session_state["tppp_logs"] = result["logs"]
                    st.session_state["tppp_result"] = result
                    st.success("Real PySpark TPPP job completed successfully.")
                except Exception as exc:  # Spark/Java failures should be visible in the frontend.
                    st.session_state["tppp_logs"] = [f"TPPP failed: {exc}"]
                    st.error(f"TPPP failed: {exc}")

        with log_col:
            st.markdown('<h4 class="section-title">Real PySpark Job Log</h4>', unsafe_allow_html=True)
            for event in st.session_state.get("tppp_logs", ["Waiting for real PySpark TPPP job..."]):
                st.code(event, language="text")

        if Path(TPPP_SUMMARY_FILE).exists() and Path(TPPP_ANOMALY_FILE).exists():
            summary_df, anomalies_df = load_tppp_outputs(str(TPPP_SUMMARY_FILE), str(TPPP_ANOMALY_FILE))
            tppp_cards = st.columns(4)
            with tppp_cards[0]:
                metric_card("Summary groups", f"{len(summary_df):,}", "Zone/weather/congestion groups")
            with tppp_cards[1]:
                metric_card("Top anomalies", f"{len(anomalies_df):,}", "Spark-ranked risk events")
            with tppp_cards[2]:
                metric_card("Max severe rate", f"{summary_df['severe_event_rate'].max():.1f}%", "Highest grouped rate")
            with tppp_cards[3]:
                metric_card("Avg Spark risk", f"{summary_df['avg_risk_index'].mean():.3f}", "Grouped mean risk")

            summary_chart_col, anomaly_table_col = st.columns((1.05, 1.2))
            with summary_chart_col:
                st.markdown('<h4 class="section-title">Spark Severe Event Rate by Zone</h4>', unsafe_allow_html=True)
                zone_rates = summary_df.groupby("city_zone", as_index=False).agg(
                    severe_event_rate=("severe_event_rate", "mean"),
                    avg_risk_index=("avg_risk_index", "mean"),
                    vehicle_events=("vehicle_events", "sum"),
                ).sort_values("severe_event_rate", ascending=False)
                fig = px.bar(
                    zone_rates,
                    x="city_zone",
                    y="severe_event_rate",
            ...
            with anomaly_table_col:
                st.markdown('<h4 class="section-title">Spark Top Risk Events</h4>', unsafe_allow_html=True)
                st.dataframe(anomalies_df.head(50), use_container_width=True, hide_index=True)

            download_cols = st.columns(2)
            with download_cols[0]:
                st.download_button(
                    "Download TPPP Summary CSV",
                    data=summary_df.to_csv(index=False),
                    file_name="traffic_zone_weather_summary.csv",
                    mime="text/csv",
                    use_container_width=True,
                )
            with download_cols[1]:
                st.download_button(
                    "Download TPPP Top Anomalies CSV",
                    data=anomalies_df.to_csv(index=False),
                    file_name="traffic_top_anomalies.csv",
                    mime="text/csv",
                    use_container_width=True,
                )
        else:
            st.info("Run the TPPP Real PySpark Job to create Spark-generated summaries and anomaly exports.")

    with icu_tab:
        st.markdown('<h3 class="section-title">Spark-style Batch Preprocessing Simulation for Large ICU Dataset</h3>', unsafe_allow_html=True)
        st.caption(
        ...
        with icu_controls:
            ...
            if st.button("Run ICU Batch Simulation", use_container_width=True):
                with st.spinner("Running simulated ICU batch preprocessing..."):
    ...
    with data_hub_tab:
        ...
        with hub_cols[1]:
            ...
            st.markdown("#### Real PySpark TPPP outputs")
            st.write(f"TPPP summary: `{TPPP_SUMMARY_FILE}`")
            st.write(f"TPPP anomalies: `{TPPP_ANOMALY_FILE}`")
            st.write(f"TPPP curated Parquet: `{TPPP_CURATED_DIR}`")
